                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
	...
	selector = ucontrol->value.integer.value[0];
		...
		if (selector == CHIP_SELECTOR_LEFT) {
			...
			tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
			tfa98xx->profile = 0;//TODO:
			tfa98xx_mixer_profile = 0;//TODO:
		}
	}
        mutex_unlock(&tfa98xx_mutex);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	if (tfa98xx->tfa->tfa_family == 2 &&
		...
		/*unnecessary of coldstart for non-dsp chip*/
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
			     ...
			     const struct i2c_device_id *id)
{
	...
	tfa98xx->vdd = devm_regulator_get(tfa98xx->dev,"tfa9874");
	if (IS_ERR_OR_NULL(tfa98xx->vdd)) {
		ret = PTR_ERR(tfa98xx->vdd);
		dev_err(tfa98xx->dev,"jzw Failed to get regulator avdd:%d\n", ret);
		tfa98xx->vdd = NULL;
		return ret;
	...
	if (tfa98xx->vdd){
		...
		if(!ret){
			dev_info(tfa98xx->dev,"jzw regulator enable SUCCESS\n");
		}else {
			dev_err(tfa98xx->dev,"jzw Failed to enable analog power:%d\n", ret);
			return ret;
		}
	}
